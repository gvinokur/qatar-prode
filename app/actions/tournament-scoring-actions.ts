...
export async function getTournamentScoringConfigAction(tournamentId: string, locale: Locale = 'es') {
  try {
    ...
    console.error('Error getting tournament scoring config:', error);
...
export async function updateTournamentScoringConfigAction(
  ...
  try {
    ...
    const result = await updateTournament(tournamentId, update);
    return { success: true, data: result };
  } catch (error) {
    console.error('Error updating tournament scoring config:', error);
    const tErrors = await getTranslations({ locale, namespace: 'errors' });
    return { success: false, error: tErrors('generic') };
  }
}
...
export async function getRecommendedScoringValues(tournamentId: string, locale: Locale = 'es') {
  try {
    ...
    console.error('Error getting recommended scoring values:', error);
    const tErrors = await getTranslations({ locale, namespace: 'errors' });
    return { success: false, error: tErrors('generic') };
  }
}
