...
Tournament Hub data fetching: shared page-level hub data, Action Center game cards, Leaderboard Peek widget, Recent Results widget, public tournament timing, and Stats at a Glance widget. Priority Attention logic lives in `app/utils/priority-attention.ts`.

**Exported types:** `StatsAtAGlanceData { hasData: boolean, totalPoints: number, matchesPoints: number, qualifiedTeamsPoints: number, awardsPoints: number, momentumPoints: number, matchesDelta: number, qualifiedTeamsDelta: number, awardsDelta: number, snapshotDateLabel: string|null, isYesterday: boolean, sparklineData: number[], sparklineStartDateLabel: string|null }`, `TournamentHubPageData { scoringConfig: ScoringConfig, totalGames: number, isStarted: boolean, isNearStart: boolean, isFinished: boolean, qualifiersTotal: number, awardsTotal: number, firstGameDate: Date|null, tournamentHasStarted: boolean, tournamentJustStarted: boolean, tournamentName: string|null }`, `TournamentTiming { firstGameDate: Date|null, tournamentHasStarted: boolean, tournamentJustStarted: boolean, tournamentName: string|null }`, `ActionCenterData { games, gameGuesses, teamsMap, tournamentMaxSilver, tournamentMaxGolden, mode: 'urgent'|'fallback'|'empty', qtAndAwardsOpen, msUntilPredictionLock, tournamentFinished, firstGameDate: Date|null, tournamentHasStarted: boolean, tournamentName: string|null, openerBackfill: boolean, totalGames: number, predictedGames: number, groupStageGamesCompleted: number, groupStageGamesTotal: number, silverBoostsUsed: number, goldenBoostsUsed: number, awardsCompleted: number, awardsTotal: number, qualifiersCompleted: number, qualifiersTotal: number, tournamentJustStarted: boolean, scoringConfig: ScoringConfig, nowAvailablePlayoffRound: { roundId: string, roundName: string, firstGameId: string } | null }`, `CarouselGamesData { games, gameGuesses, teamsMap, tournamentMaxSilver, tournamentMaxGolden, mode: 'urgent'|'fallback'|'empty', urgentGameIds: string[], predictedGames: number, silverBoostsUsed: number, goldenBoostsUsed: number }`, `LeaderboardPeekResult { groups: GroupPeekData[], userHasGroups: boolean, allGroupNames: Array<{ id: string; name: string }> }`, `RankNeighborEntry { userId, userName, rank, score, isCurrentUser }`, `GroupPeekData { groupId, groupName, totalMembers, userRank, rankChange: number|null, rows: RankNeighborEntry[] }`, `HonorRollPosition = 'champion'|'runnerUp'|'thirdPlace'`, `IndividualAwardType = 'bestPlayer'|'topGoalscorer'|'bestGoalkeeper'|'bestYoungPlayer'`, `GameStatus = 'finished'|'pending'|'about_to_start'`, `RecentGameResultItem { gameId, homeTeamName, awayTeamName, homeScore: number|null, awayScore: number|null, homePenaltyScore: number|null, awayPenaltyScore: number|null, userHomeGuess: number|null, userAwayGuess: number|null, userHomePenaltyWinner: boolean|null, userAwayPenaltyWinner: boolean|null, basePoints, boostType: 'silver'|'golden'|null, boostBonus, finalPoints, predictionTier: 'exact'|'goal_difference'|'correct'|'missed'|null, gameDate, gameStatus: GameStatus }`, `RecentResultsData { recentGames: RecentGameResultItem[] }`

- **getTournamentHubPageData(tournamentId: string, locale: Locale)**: `Promise<TournamentHubPageData>` — Server Action. Returns shared tournament data AND timing fields needed by all dashboard widgets and hero banners. Does NOT require authentication. Runs `findTournamentById`, `findFirstGameInTournament`, `findLastGameInTournament`, and an inline Kysely `countAll` query for total games in parallel. Derives `isStarted`, `isNearStart`, `isFinished` from game dates; computes `tournamentHasStarted` (first game kicked off), `tournamentJustStarted` (kicked off within last 48h), `firstGameDate`, and `tournamentName` via `computeTournamentName`. Falls back to DEFAULT_SCORING when tournament is not found or all scoring fields are null. Eliminates the former `getPublicTournamentTiming` call site.
  Calls: findTournamentById, findFirstGameInTournament, findLastGameInTournament, db.selectFrom (inline count), computeTournamentName
- **getActionCenterGames(tournamentId: string, locale: Locale)**: `Promise<ActionCenterData>` — Server Action. Fetches games in the 7-day dashboard window, user guesses, teams, tournament boost limits, first/last game dates, and prediction completion stats via `getTournamentPredictionCompletion`. Also fetches playoff round availability in parallel and computes `nowAvailablePlayoffRound` (first qualifying playoff round with its localized name and first unpredicted game ID; null when none qualifies). Returns up to 5 unpredicted open-deadline games (urgent mode, secondary sorted by game_number), or next 5 games within a 5-day lookahead window when all open games are predicted (fallback mode, secondary sorted by game_number), or empty/fallback mode when no window games exist. When no window games are found and the tournament hasn't started, backfills `games` with the opener via `findFirstGameFullData` and sets `openerBackfill=true` (mode becomes 'fallback'). Computes: `qtAndAwardsOpen`/`msUntilPredictionLock` (lock fires 5 days after first game); `tournamentFinished` (last game has kicked off); `tournamentHasStarted` (first game has kicked off); `firstGameDate`; `tournamentName` (localized short name via `applyLocalization`); `totalGames`, `predictedGames`, `groupStageGamesCompleted`, `groupStageGamesTotal`, `silverBoostsUsed`, `goldenBoostsUsed`, `awardsCompleted`, `awardsTotal`, `qualifiersCompleted`, `qualifiersTotal` (from prediction completion); `tournamentJustStarted` (first game kicked off within last 48h). Throws Unauthorized if user is not logged in.
  Calls: getLoggedInUser, findGamesForDashboard, findGameGuessesByUserId, findTeamInTournament, findTournamentById, findFirstGameInTournament, findLastGameInTournament, findPlayoffRoundsWithAvailabilityInfo, getTournamentPredictionCompletion, findPlayoffRoundBy (conditional), findFirstGameFullData (conditional), computeNowAvailableRoundIds, calculateDeadline, applyLocalizationBatch, applyLocalization
- **getPlayoffRoundsAvailability(tournamentId: string)**: `Promise<string[]>` — Server Action. Lightweight action for the Games page. Calls `findPlayoffRoundsWithAvailabilityInfo` and `computeNowAvailableRoundIds`. Returns array of now-available round IDs. Throws Unauthorized when no active session.
  Calls: getLoggedInUser, findPlayoffRoundsWithAvailabilityInfo, computeNowAvailableRoundIds
- **getCarouselGames(tournamentId: string, locale: Locale)**: `Promise<CarouselGamesData>` — Server Action. Lightweight carousel-only refetch used by `GamesActiveSection` when all urgent games become complete. Does NOT call `getTournamentPredictionCompletion` — instead runs three aggregate queries in parallel (predicted count, silver boost count, golden boost count) alongside the main data queries. Same urgent/fallback/empty game selection logic as `getActionCenterGames` (up to 5 urgent cards, up to 5 fallback cards within 5-day window, secondary sorted by game_number). Includes `urgentGameIds` (IDs of selected urgent games, empty in fallback/empty mode). Throws Unauthorized if user is not logged in.
  Calls: getLoggedInUser, findGamesForDashboard, findGameGuessesByUserId, findTeamInTournament, findTournamentById, db.selectFrom (3 inline aggregate queries), calculateDeadline, applyLocalizationBatch
- **getLeaderboardPeekData(tournamentId: string, _locale: Locale)**: `Promise<LeaderboardPeekResult>` — Server Action. Returns up to 3 friend groups where the current user has ranking data, sorted by favorites first then ranked member count descending. Uses a two-phase approach: Phase 1 calls `getGroupRankingSummaries` (2 DB round-trips for all N groups) to get ranked member counts and user presence; filters/sorts/takes top 3 from the summary. Phase 2 fetches full rankings and rank-change snapshots only for the top 3 groups in parallel. Also returns `userHasGroups` (true if user belongs to any group, before ranking filter) and `allGroupNames` (all group names for pre-tournament preview). Per group: builds a 3-row neighbor window and computes rank change from the two most recent snapshots. Returns `{ groups: [], userHasGroups: false, allGroupNames: [] }` when unauthenticated or user has no groups.
  Calls: getLoggedInUser, findProdeGroupsByOwner, findProdeGroupsByParticipant, getFavoriteGroupIds, getGroupRankingSummaries, getLatestRankingsForGroup, getLatestTwoGroupRankingSnapshots
- **getRecentResultsData(tournamentId: string, locale: Locale)**: `Promise<RecentResultsData>` — Server Action. Fetches all games where the prediction deadline has passed (`game_date <= now() + 1h`), including games the user did not predict (null guesses). Computes `gameStatus` per game: `finished` if homeScore != null; `about_to_start` if gameDate > now(); else `pending`. Returns up to 10 games ordered by game_date desc. Throws Unauthorized if no session.
  Calls: getLoggedInUser, findRecentGamesForDashboard, findTeamInTournament, applyLocalizationBatch
- **getStatsAtAGlanceData(tournamentId: string, locale: Locale)**: `Promise<StatsAtAGlanceData>` — Server Action. Fetches score history for the logged-in user and returns a compact summary for the Stats at a Glance widget. Returns `hasData=false` with all zeros when no history exists. Computes three category totals: `matchesPoints = total_game_score + total_boost_bonus`; `qualifiedTeamsPoints = qualified_teams_score + group_position_score`; `awardsPoints = honor_roll_score + individual_awards_score`. Computes per-category deltas and `momentumPoints` from latest vs. previous snapshot (0 when only one snapshot). `isYesterday=true` when previous snapshot date equals yesterday's YYYYMMDD in Argentina TZ. `sparklineData` contains the last 7 `total_points` values in chronological order. `sparklineStartDateLabel` is the formatted date of the first snapshot in the sparkline window (null when fewer than 2 sparkline entries). Throws Unauthorized if user is not logged in.
