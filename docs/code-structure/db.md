...
- **getGameGuessStatisticsForUsers(userIds: string[], tournamentId: string)**: `Promise<GameStatisticForUser[]>` — Reads materialized game scores and accuracy counts from tournament_guesses table, including game scores, boost bonuses, prediction accuracy tiers, and tournament-level fields (`qualified_teams_score`, `qualified_teams_correct`, `qualified_teams_exact`, `honor_roll_score`, `individual_awards_score`, `group_position_score`). Single query replaces the former need for a separate `findTournamentGuessByUserIdTournament` call on the stats page.
...
- **findFirstGameInTournament(tournamentId: string)**: `Promise<Game | undefined>` — Finds first game by date (secondary sort by game_number, cached).
...
- **findLastGameInTournament(tournamentId: string)**: `Promise<Game | undefined>` — Finds last game by date (secondary sort by game_number, cached). Used for score history chart X-axis end bound.
...
- **findGamesForDashboard(tournamentId: string)**: `Promise<ExtendedGameData[]>` — Unified dashboard query: only games whose prediction deadline hasn't closed (`game_date >= now + 1h`), up to 7 days ahead (secondary sort by game_number, cached). `gameResult` is `null` when only a draft result exists (`is_draft = false` filter applied in subquery).
- **getAllTournamentGames(tournamentId: string)**: `Promise<ExtendedGameData[]>` — Fetches ALL games for unified games page (cached). Sorted by `matchday ASC NULLS LAST, game_date ASC, game_number ASC`. `gameResult` is `null` when only a draft result exists (`is_draft = false` filter applied in subquery).
...
- **findRecentGamesForDashboard(userId: string, tournamentId: string, limit: number)**: `Promise<RecentGameForDashboard[]>` — Returns all games where `game_date <= now() + 1h`, LEFT JOINing game_guesses (nullable) and published game_results (is_draft=false, nullable). Ordered by game_date desc and game_number desc, limited to `limit`. Returns empty array when limit is 0. Interface: `RecentGameForDashboard { gameId, homeTeamId, awayTeamId, homeScore: number|null, awayScore: number|null, homePenaltyScore: number|null, awayPenaltyScore: number|null, userHomeGuess: number|null, userAwayGuess: number|null, userHomePenaltyWinner: boolean|null, userAwayPenaltyWinner: boolean|null, guessScore: number|null, boostType: BoostType, boostMultiplier: number|null, finalScore: number|null, predictionTier: 'exact'|'goal_difference'|'correct'|'missed'|null, gameDate: Date }`.
...
Repository for tournament prediction completion tracking. Runs 4 parallel DB queries via `Promise.all` instead of 13 sequential ones.

- **getTournamentPredictionCompletion(userId: string, tournamentId: string, tournament: Tournament, firstGameDate?: Date | null)**: `Promise<TournamentPredictionCompletion>` — Calculates overall prediction progress including game completion, boost usage, award selection, qualifier selections, and per-playoff-round completion (`playoffRoundsCompletion: Record<roundId, PlayoffRoundCompletionData>`). Also returns `completedGroupGames` and `totalGroupGames` for QT banner state derivation. When `firstGameDate` is provided, skips the `getTournamentStartDate` DB call.
