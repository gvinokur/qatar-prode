...
- **LocaleLayout({ children, params })**: `JSX.Element` — [Server] Sets up locale context, theme providers, session wrapper, header, footer, and AdSense Auto Ads. AdSense Script loaded afterInteractive when NEXT_PUBLIC_ADSENSE_CLIENT_ID is set. Mounts AdSensePageViewTracker to signal virtual page views on SPA navigation.
