...
**Date:** 2026-05-19
**Reason:** Mixing `getTranslations` (next-intl, reads Next.js request context) with `getLoggedInUser()` (NextAuth, reads cookies via `next/headers`) in the same `Promise.all` within a Server Action causes interference — betting config save broke silently in production. Discovered during Vercel Preview testing.
